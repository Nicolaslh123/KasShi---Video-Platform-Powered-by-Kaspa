...
function mapVideoFromApi(data: Record<string, unknown>): Video {
  ...
  return {
    ...
    directVideoUrl: (data.directVideoUrl ?? data.direct_video_url) as string | null,
    ...
    isClip: Boolean(data.isClip ?? data.is_clip),
...
export function useChannelTiers(handle: string | undefined) {
    ...
    fetch(`/api/kasshi/channels/${handle}/tiers`)
      ...
      .then((data) => {
        ...
        const mappedTiers = (data.tiers || []).map((t: Record<string, unknown>) => ({
          ...
          priceUsd: t.priceUsd as number | null,
  ...
  return { tiers, kaspaPrice, loading, refetch };
}

// Check user's membership status for a channel
export function useMembershipStatus(handle: string | undefined, isLoggedIn: boolean, authToken?: string) {
  ...
  const refetch = useCallback(() => {
    ...
    fetch(`/api/kasshi/channels/${handle}/membership`, { credentials: 'include', headers })
      .then((res) => res.json())
      .then((data) => {
        ...
        if (data.isMember) {
          setMembership({
            ...
            memberChannelId: 0, // Determined by backend from auth
