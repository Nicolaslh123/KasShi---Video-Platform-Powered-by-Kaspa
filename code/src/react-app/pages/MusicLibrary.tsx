...
import { ThumbsUp, ListMusic, Radio, Clock, Music2, Play, ArrowLeft, Loader2, Plus, Trash2, Shuffle, MessageSquare, PieChart, TrendingUp } from 'lucide-react';
...
type TabType = 'liked' | 'library' | 'radio' | 'history' | 'reviews' | 'shares';
...
export default function MusicLibrary() {
  ...
  const getInitialTab = (): TabType => {
    if (tab === 'liked' || tab === 'library' || tab === 'radio' || tab === 'history' || tab === 'reviews' || tab === 'shares') {
      return tab;
    }
    return 'liked';
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [likedRes, historyRes, radioRes, playlistsRes, likedPlaylistsRes, reviewsRes, reviewedTracksRes, sharesRes] = await Promise.all([
          ...
          timedFetch('/api/music/user/shares', { headers, credentials: 'include' }),
        ]);
  ...
  const tabs: { id: TabType; label: string; icon: typeof ThumbsUp }[] = [
    ...
    { id: 'shares', label: 'My Shares', icon: PieChart },
  ];
  ...
  const getEmptyMessage = () => {
    switch (activeTab) {
      ...
      case 'shares': return "Your track ownership shares will appear here. Buy shares in fractionalized tracks to earn royalties.";
      default: return "";
    }
  };
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="sm:hidden">{tab.id === 'liked' ? 'Liked' : tab.id === 'library' ? 'Library' : tab.id === 'radio' ? 'Radio' : tab.id === 'history' ? 'History' : tab.id === 'reviews' ? 'Reviews' : 'Shares'}</span>
                </button>
              );
            })}
          </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-4">
                <div className="flex items-center gap-2 sm:gap-4">
                  <div className="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 rounded-full text-white font-semibold text-sm sm:text-base" style={{ backgroundColor: `${theme.accent}30` }}>
                    <PieChart className="w-4 h-4" style={{ color: theme.accent }} />
                    <span>Portfolio</span>
                  </div>
                  <span className="text-white px-2 sm:px-3 py-1 rounded-full bg-black/50 text-sm">{shares.length} holdings</span>
                </div>
                <LocalizedLink
                  to="/music/investments"
                  className="flex items-center gap-2 px-4 py-2 rounded-full bg-purple-600 hover:bg-purple-500 text-white text-sm font-medium transition-colors"
                >
              ...
              {shares.length === 0 ? (
                <div className="rounded-xl sm:rounded-2xl p-[1px]" style={{ background: `linear-gradient(135deg, ${theme.accent}40 0%, transparent 50%, ${theme.accent}20 100%)` }}>
                  <div className="bg-black/70 rounded-xl sm:rounded-2xl p-6 sm:p-8 text-center">
                    ...
                    <h2 className="text-lg sm:text-xl font-bold text-white mb-2">My Shares</h2>
                    <p className="text-white/60 text-sm sm:text-base">{getEmptyMessage()}</p>
                    <LocalizedLink
                      to="/music"
                      className="inline-block mt-4 px-5 sm:px-6 py-2 rounded-full text-black font-medium text-sm sm:text-base"
                    ...
                    <div className="grid gap-1.5 sm:gap-2 p-3 sm:p-4">
                      {shares.map((share) => (
                        <LocalizedLink
                          ...
                          className="flex items-center gap-3 sm:gap-4 p-2.5 sm:p-3 rounded-lg sm:rounded-xl bg-white/5 hover:bg-white/10 transition-colors"
                        >
                          <div 
                            className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg flex items-center justify-center flex-shrink-0"
                            style={{ backgroundColor: `${theme.accent}30` }}
                          >
                            {share.trackCover ? (
                              <img src={share.trackCover} alt="" className="w-full h-full object-cover rounded-lg" />
                            ) : (
                              <Music2 className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: theme.accent }} />
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <h3 className="font-medium text-white truncate text-sm sm:text-base">{share.trackTitle}</h3>
                              <span className="text-[10px] sm:text-xs px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-400 font-mono flex-shrink-0">
                                ${share.ticker}
                              </span>
                            </div>
                            <p className="text-xs sm:text-sm text-white/50 mb-0.5">{share.artistName}</p>
                            <div className="flex items-center gap-3 text-xs text-white/40">
                              <span className="font-medium" style={{ color: theme.accent }}>
                                {share.sharesOwned.toLocaleString()} shares
                              </span>
                              <span>
                                ({((share.sharesOwned / share.totalShares) * 100).toFixed(2)}% ownership)
                              </span>
                            </div>
                          </div>
                          <div className="text-right flex-shrink-0">
                            <p className="text-xs sm:text-sm font-medium text-white/70">
                              {share.purchasePriceKas} KAS
                            </p>
                            <p className="text-[10px] sm:text-xs text-white/40">
                              {new Date(share.purchasedAt).toLocaleDateString()}
                            </p>
                          </div>
                        </LocalizedLink>
                      ))}
