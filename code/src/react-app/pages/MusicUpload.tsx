...
import { generateBeatGrid } from '../utils/generateBeatGrid';
import toast from 'react-hot-toast';

// Compute SHA-256 hash of audio file for fractionalization protection
...
export default function MusicUpload() {
  ...
  // TEMP DISABLED: _pay is used for upload fees
  const { pay: _pay, canPay } = usePayment();
  ...
  // Helper to check if file is audio (checks both MIME type and extension for mobile/Brave compatibility)
  const isAudioFile = (file: File): boolean => {
    // Check MIME type first
    if (file.type.startsWith('audio/')) return true;
    
    // Fallback to extension check (mobile browsers often don't set MIME type correctly)
    const audioExtensions = ['.mp3', '.wav', '.flac', '.m4a', '.aac', '.ogg', '.wma', '.aiff', '.opus'];
  ...
  const handlePaymentAndUpload = async () => {
    ...
    const _fee = calculateUploadFee();
    // @ts-ignore - temporarily unused while upload payments are disabled
    const _balanceNum = parseFloat(balance) || 0;
    ...
    try {
      ...
      if (uploadType === 'track') {
        if (audioFile) {
          ...
          // Compute audio hash for fractionalization protection
          setUploadStatus('Computing audio fingerprint...');
          try {
            const hash = await computeAudioHash(audioFile);
            (window as any).__uploadAudioHash = hash;
            console.log('✅ Audio hash computed');
          } catch (e) {
            console.warn('⚠️ Audio hash computation failed:', e);
          }
          
          // Analyze beats for reactive visualizer
          setUploadStatus('Analyzing beats with Essentia.js (4-12 seconds)...');
          setUploadProgress(35);
          try {
            const beatResult = await generateBeatGrid(audioFile);
            if (beatResult.beatGrid.length > 0) {
              (window as any).__uploadBeatGrid = beatResult.beatGrid;
              (window as any).__uploadBpm = beatResult.bpm;
              console.log(`✅ Essentia Beat Grid: ${beatResult.beatGrid.length} beats @ ${beatResult.bpm} BPM`);
            }
            setUploadStatus('Beat analysis complete → saving track...');
          } catch (e) {
            console.warn('⚠️ Essentia analysis failed, uploading without beat grid:', e);
            setUploadStatus('Uploading without beat grid (real-time fallback will be used)');
          }
        }
      } else if (uploadType === 'album') {
        // Upload all album tracks and analyze beats
        const albumBeatGrids: { [key: string]: { beatGrid: number[]; bpm: number } } = {};
        for (let i = 0; i < albumTracks.length; i++) {
          const track = albumTracks[i];
          if (track.file) {
            setUploadStatus(`Uploading track ${i + 1} of ${albumTracks.length}...`);
            setUploadProgress(10 + Math.floor((i + 1) / albumTracks.length * 30));
            uploadedTrackUrls[track.id] = await uploadFileToR2(track.file);
            
            // Analyze beats for each track
            setUploadStatus(`Analyzing beats for track ${i + 1} (4-12 seconds)...`);
            try {
              const beatResult = await generateBeatGrid(track.file);
              if (beatResult.beatGrid.length > 0) {
                albumBeatGrids[track.id] = beatResult;
                console.log(`✅ Track ${i + 1}: ${beatResult.beatGrid.length} beats @ ${beatResult.bpm} BPM`);
              }
            } catch (e) {
              console.warn(`⚠️ Beat analysis failed for track ${i + 1}:`, e);
            }
          }
        }
        (window as any).__albumBeatGrids = albumBeatGrids;
      } else if (uploadType === 'podcast') {
      ...
      // TEMP DISABLED: Payment processing
  ...
  const saveToDatabase = async (
    ...
    try {
      if (uploadType === 'track') {
        ...
        // Get beat analysis results if available
        const beatGrid = (window as any).__uploadBeatGrid as number[] | undefined;
        const bpm = (window as any).__uploadBpm as number | undefined;
        const audioHash = (window as any).__uploadAudioHash as string | undefined;
        // Clean up temp storage
        delete (window as any).__uploadBeatGrid;
        delete (window as any).__uploadBpm;
        delete (window as any).__uploadAudioHash;
        ...
        // Get album beat grids
        const albumBeatGrids = (window as any).__albumBeatGrids as { [key: string]: { beatGrid: number[]; bpm: number } } | undefined;
        delete (window as any).__albumBeatGrids;
        ...
        for (let i = 0; i < albumTracks.length; i++) {
          ...
          const trackBeatData = albumBeatGrids?.[track.id];
          ...
          await createTrack({
            ...
            isExplicit: false,
            albumId,
            beatGrid: trackBeatData?.beatGrid,
            bpm: trackBeatData?.bpm,
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-4 pt-20 sm:pt-24 pb-24 sm:pb-32 relative z-10">
        ...
        {step === 'details' && (
          <div className="bg-white/5 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8">
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <input
                            ref={audioInputRef}
                            type="file"
                            accept="audio/*,.mp3,.wav,.flac,.m4a,.aac,.ogg,.wma,.aiff,.opus"
            ...
            {/* TEMP DISABLED: Upload Fee Display - hidden while upload payments are disabled */}
            {false && (
            ...
            <div className="mt-6 flex justify-end gap-4">
              ...
              <button
                ...
                disabled={!hasRequiredFields || !canPay || isProcessingPayment}
                ...
                style={{ 
                  backgroundColor: hasRequiredFields && canPay ? theme.accent : 'rgba(255,255,255,0.1)',
                  color: hasRequiredFields && canPay ? 'black' : 'rgba(255,255,255,0.4)'
                }}
              >
                {isProcessingPayment ? (
                  <>
                  ...
                  <>
                    ...
                    {/* TEMP DISABLED: Changed from "Pay & Upload" while payments disabled */}
                    Upload {uploadType === 'album' ? 'Album' : uploadType === 'podcast' ? 'Episode' : 'Track'}
                  </>
                )}
              </button>
            </div>
