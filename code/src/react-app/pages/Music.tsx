...
import { Music2, Mic2, Compass, ListMusic, Clock, ThumbsUp, Palette, Play, Pause, AlertCircle, Video, Upload, Loader2, User, X, Wallet, Search, Trophy, LayoutDashboard, Eye, EyeOff, PieChart } from 'lucide-react';
...
export default function Music() {
  ...
  // Handle opening theme selector - calculate position first, then show
    ...
    <div className="min-h-screen relative w-full overflow-x-hidden">
      ...
      {showDisclaimer && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-white/20 rounded-2xl max-w-lg w-full p-6 shadow-2xl my-auto">
          ...
          <div className="max-w-[2000px] mx-auto px-4 py-3 sm:py-4">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5 sm:gap-3">
                ...
                {/* Marketplace button - temporarily hidden */}
                {/* <LocalizedLink 
                  to="/music/marketplace" 
                  className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full font-medium text-sm transition-colors bg-purple-600/80 hover:bg-purple-500 text-white"
                >
              ...
              {/* Right side */}
              <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0 max-w-[60%] sm:max-w-none overflow-x-auto scrollbar-hide">
                {/* Mobile search button - navigates to search page */}
                <LocalizedLink
                  to="/music/search"
                  className="md:hidden p-1.5 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-white/80 hover:text-white"
                  title="Search"
                >
                ...
                {(wallet?.address || externalWallet?.address || externalWallet?.authToken || mochaUser) && (
                  <LocalizedLink 
                    to="/dashboard" 
                    className="flex p-1.5 sm:p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-white/80 hover:text-white"
                    title="Dashboard"
                  >
                ...
                <div className="relative">
                  <button
                    ref={themeButtonRef}
                    onClick={() => showThemeSelector ? setShowThemeSelector(false) : handleOpenThemeSelector()}
                  ...
                  {showThemeSelector && createPortal(
                    <>
                      ...
                      />
                      <div 
                        className="fixed w-56 bg-slate-900/95 backdrop-blur-xl rounded-xl border border-white/10 shadow-2xl overflow-hidden z-[9999] animate-in fade-in duration-100"
                ...
                <div className="sm:hidden">
                  {/* Mobile Marketplace button - temporarily hidden */}
                  {/* <LocalizedLink 
                    to="/music/marketplace" 
                    className="flex p-1.5 rounded-full bg-purple-600/80 text-white hover:bg-purple-500 transition-colors mr-1"
                    title="Market"
                  >
                    <Store className="w-4 h-4" />
                  </LocalizedLink> */}
              ...
              <section className="mb-6 sm:mb-12">
                <div className="rounded-xl sm:rounded-2xl p-[1px]" style={{ background: `linear-gradient(135deg, ${theme.accent}40 0%, transparent 50%, ${theme.accent}20 100%)` }}>
                  <div className="bg-black/70 rounded-xl sm:rounded-2xl p-2 sm:p-6">
                    <div className="flex sm:grid sm:grid-cols-6 gap-1 sm:gap-4 overflow-x-auto pb-1 sm:pb-0 scrollbar-hide">
                      ...
                      {/* TEMP DISABLED - Shares button */}
                      {false && (
                      <LocalizedLink to="/music/investments" className="flex-shrink-0 w-[72px] sm:w-auto flex flex-col items-center gap-1 sm:gap-3 bg-white/5 hover:bg-white/10 rounded-lg sm:rounded-xl p-2 sm:p-4 transition-colors group">
                        <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${theme.accent}20` }}>
                          <PieChart className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: theme.accent }} />
                        </div>
                        <div className="text-center">
                          <p className="text-white font-medium text-[10px] sm:text-base">Shares</p>
                        </div>
                      </LocalizedLink>
                      )}
