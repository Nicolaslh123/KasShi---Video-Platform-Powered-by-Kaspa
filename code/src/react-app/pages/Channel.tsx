...
export default function Channel() {
  ...
  const { tiers, kaspaPrice, loading: tiersLoading, refetch: refetchTiers } = useChannelTiers(channelId);
  ...
  const { membership, refetch: refetchMembership } = useMembershipStatus(channelId, isLoggedIn, externalWallet?.authToken);
  ...
  const isChannelOwner = Boolean(viewerChannel?.id && channel?.id && viewerChannel.id === channel.id);
  const { videos: myVideos, loading: myVideosLoading } = useMyVideos(channelId, isChannelOwner, externalWallet?.authToken);
  
  const [activeTab, setActiveTab] = useState<"videos" | "my-videos" | "membership" | "about" | "liked">(() => {
    const tab = searchParams.get("tab");
    if (tab === "membership" || tab === "about" || tab === "my-videos" || tab === "liked") {
      return tab;
    }
    return "videos";
  });
  ...
  const [newTierPriceUsd, setNewTierPriceUsd] = useState("");
  const [isUsdPricing, setIsUsdPricing] = useState(true); // Default to USD
  ...
  const [isEditUsdPricing, setIsEditUsdPricing] = useState(true);
  ...
  }, [pendingMembership, channelId, channel, isExternalWallet, unifiedPay, externalWallet, refetchMembership, refetchTiers]);

  const handleCreateTier = async () => {
    const priceInput = isUsdPricing ? newTierPriceUsd : newTierPrice;
    if (!newTierName.trim() || !priceInput) {
    ...
    // For USD pricing, validate minimum KAS equivalent
    if (isUsdPricing && kaspaPrice) {
      const kasEquivalent = price / kaspaPrice;
      if (kasEquivalent < 0.11) {
        toast.error(`Minimum tier price is ~$${(0.11 * kaspaPrice).toFixed(2)} USD (0.11 KAS)`);
        return;
      }
    }
    ...
    try {
      ...
      // Build request body based on pricing mode
      ...
      if (isUsdPricing) {
        requestBody.priceUsd = price;
      } else {
        requestBody.priceKas = price;
      }
  ...
  const handleEditTier = (tier: MembershipTier) => {
    ...
    setEditTierBenefits(tier.benefits.join("\n"));
    if (tier.priceUsd) {
      setIsEditUsdPricing(true);
      setEditTierPriceUsd(tier.priceUsd.toString());
      setEditTierPrice("");
    } else {
      setIsEditUsdPricing(false);
      setEditTierPrice(tier.priceKas);
      setEditTierPriceUsd("");
    }
  ...
  const handleSaveTier = async () => {
    if (!editingTier || !channelId) return;
    
    const price = isEditUsdPricing ? parseFloat(editTierPriceUsd) : parseFloat(editTierPrice);
    if (isNaN(price) || price <= 0) {
    ...
    try {
      const benefits = editTierBenefits
        .split("\n")
        .map(b => b.trim())
        .filter(b => b.length > 0);
      ...
      const requestBody: Record<string, unknown> = {
        ...
        description: editTierDescription.trim() || null,
        benefits,
      };
      ...
      if (isEditUsdPricing) {
        ...
        requestBody.priceKas = price.toString();
      }
      
      const res = await fetch(`/api/kasshi/channels/${channelId}/tiers/${editingTier.id}`, {
        method: "PATCH",
        headers: tierHeaders,
        credentials: "include",
        body: JSON.stringify(requestBody),
  ...
  const handleDeleteTier = async (tier: MembershipTier) => {
    ...
    try {
      ...
      const res = await fetch(`/api/kasshi/channels/${channelId}/tiers/${tier.id}`, {
        method: "DELETE",
        headers: tierHeaders,
        credentials: "include",
      });
  ...
  return (
    <div className={`min-h-screen w-full bg-slate-950 flex flex-col ${titleBarPadding}`}>
                ...
                <div className="relative" data-share-dropdown>
                  ...
                  {showShareMenu && (
                    <div className="absolute right-0 mt-2 w-48 bg-slate-800 rounded-xl border border-slate-700 shadow-xl z-50 overflow-hidden">
                      <button
                        onClick={() => {
                          const url = `${window.location.origin}/video/channel/${channel?.handle}`;
                          ...
                          {isChannelOwner && (
                            <div className="flex gap-2 mt-4">
                              <button
                                ...
                                className="flex-1 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-1"
                              >
                                <Pencil className="w-4 h-4" />
                                Edit
                              </button>
                              <button
                                ...
                                className="flex-1 py-2 bg-red-600/20 hover:bg-red-600/30 text-red-400 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-1 disabled:opacity-50"
                              >
      ...
      <SecurityVerificationModal
        ...
        amountUsd={pendingMembership?.priceUsd}
      />
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm text-slate-400 mb-2 block">{t.channel.tierPrice} *</label>
                
                {/* USD/KAS Toggle */}
                <div className="flex gap-2 mb-3">
                  <button
                    type="button"
                    onClick={() => setIsUsdPricing(true)}
                ...
                {isUsdPricing ? (
                  <div className="space-y-2">
                    <div className="relative">
                      <div className="absolute left-4 top-1/2 -translate-y-1/2 text-green-400 font-semibold">$</div>
                      <input
                        type="number"
                        value={newTierPriceUsd}
                        onChange={e => setNewTierPriceUsd(e.target.value)}
                        placeholder="5.00"
                        step="0.01"
                        min="0.01"
                        className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
                      />
                    </div>
                    {newTierPriceUsd && kaspaPrice && (
                      <div className="flex items-center gap-1.5 text-sm text-slate-400 bg-slate-800/50 rounded-lg px-3 py-2">
                        <KaspaIcon size={14} />
                        <span>≈ {(parseFloat(newTierPriceUsd) / kaspaPrice).toFixed(2)} KAS</span>
                        <span className="text-slate-500 ml-auto text-xs">@ ${kaspaPrice.toFixed(4)}/KAS</span>
                      </div>
                    )}
                    <p className="text-xs text-slate-500">KAS amount updates automatically with exchange rate</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <div className="relative">
                    ...
                    {newTierPrice && kaspaPrice && (
                      <div className="flex items-center gap-1.5 text-sm text-slate-400 bg-slate-800/50 rounded-lg px-3 py-2">
                        <span className="text-green-400">$</span>
                        <span>≈ ${(parseFloat(newTierPrice) * kaspaPrice).toFixed(2)} USD</span>
                      </div>
                    )}
                    <p className="text-xs text-slate-500">{t.channel.tierPriceHint || 'Fixed KAS price (31 days access)'}</p>
                  </div>
                )}
              </div>
            ...
            <button
              ...
              disabled={isCreatingTier || !newTierName.trim() || (isUsdPricing ? !newTierPriceUsd : !newTierPrice)}
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <button
                    type="button"
                    onClick={() => setIsEditUsdPricing(true)}
                    className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-all ${
                      isEditUsdPricing 
                        ? 'bg-teal-500/20 text-teal-400 border border-teal-500/50' 
                        : 'bg-slate-800 text-slate-400 border border-slate-700 hover:border-slate-600'
                  ...
                  <button
                    type="button"
                    onClick={() => setIsEditUsdPricing(false)}
                    className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-1.5 ${
                      !isEditUsdPricing 
                        ? 'bg-teal-500/20 text-teal-400 border border-teal-500/50' 
                        : 'bg-slate-800 text-slate-400 border border-slate-700 hover:border-slate-600'
                ...
                {isEditUsdPricing ? (
                  <div className="space-y-2">
                    ...
                    {editTierPriceUsd && kaspaPrice && (
                      <div className="flex items-center gap-1.5 text-sm text-slate-400 bg-slate-800/50 rounded-lg px-3 py-2">
                        <KaspaIcon size={14} />
                        <span>≈ {(parseFloat(editTierPriceUsd) / kaspaPrice).toFixed(2)} KAS</span>
                        <span className="text-slate-500 ml-auto text-xs">@ ${kaspaPrice.toFixed(4)}/KAS</span>
                      </div>
                    )}
                    <p className="text-xs text-slate-500">KAS amount updates automatically with exchange rate</p>
                  ...
                  <div className="space-y-2">
                    ...
                    {editTierPrice && kaspaPrice && (
                      <div className="flex items-center gap-1.5 text-sm text-slate-400 bg-slate-800/50 rounded-lg px-3 py-2">
                        <span className="text-green-400">$</span>
                        <span>≈ ${(parseFloat(editTierPrice) * kaspaPrice).toFixed(2)} USD</span>
                      </div>
                    )}
                    <p className="text-xs text-slate-500">{t.channel.tierPriceHint || 'Fixed KAS price (31 days access)'}</p>
                  </div>
            ...
            <button
              ...
              disabled={isSavingTier || !editTierName.trim() || (isEditUsdPricing ? !editTierPriceUsd : !editTierPrice)}
              className="w-full mt-6 py-3 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 disabled:from-slate-700 disabled:to-slate-600 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2"
            >
              {isSavingTier ? (
                <>
