    ...
    <div className="space-y-3">
      <div className="flex gap-2 sm:gap-3">
        <img 
          ...
          alt={c.author.name}
          className={`rounded-full object-cover flex-shrink-0 ${depth === 0 ? "w-8 h-8 sm:w-10 sm:h-10" : "w-7 h-7 sm:w-8 sm:h-8"}`}
        />
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 sm:gap-3 mt-2 -ml-1.5 sm:ml-0">
            <button 
              onClick={() => handleCommentLike(String(c.id), c.author.walletAddress, c.author.name)}
              disabled={likingCommentId === String(c.id)}
              className={`flex items-center gap-1 transition-colors text-xs p-1.5 sm:p-0 min-h-[36px] sm:min-h-0 ${likingCommentId === String(c.id) ? "opacity-50 cursor-wait" : ""} ${isLiked ? "text-teal-400" : "text-slate-400 active:text-teal-400 sm:hover:text-teal-400"}`}
            ...
            <button 
              ...
              className={`flex items-center gap-1 transition-colors text-xs p-1.5 sm:p-0 min-h-[36px] sm:min-h-0 ${dislikingCommentId === String(c.id) ? "opacity-50 cursor-wait" : ""} ${isDisliked ? "text-orange-400" : "text-slate-400 active:text-orange-400 sm:hover:text-orange-400"}`}
            ...
            <button 
              ...
              className="flex items-center gap-1 text-slate-400 active:text-teal-400 sm:hover:text-teal-400 transition-colors text-xs p-1.5 sm:p-0 min-h-[36px] sm:min-h-0"
            >
            ...
            {currentUserChannelId === c.author.id && (
              <button 
                ...
                className={`flex items-center gap-1 text-slate-400 active:text-red-400 sm:hover:text-red-400 transition-colors text-xs p-1.5 sm:p-0 min-h-[36px] sm:min-h-0 ${deletingCommentId === String(c.id) ? "opacity-50 cursor-wait" : ""}`}
          ...
          {replyingTo?.id === c.id && (
            ...
            >
              <CornerDownRight className="w-4 h-4 text-slate-500 flex-shrink-0 mt-2.5 sm:mt-2 hidden xs:block" />
              <div className="flex-1">
                <input
                  ...
                  className="w-full bg-slate-800/50 border border-slate-700 focus:border-teal-500 rounded-lg px-3 py-2.5 sm:py-2 text-white text-sm placeholder-slate-500 focus:outline-none transition-colors"
                  autoFocus
                ...
                <div className="flex justify-end gap-2 mt-2">
                  <button 
                    ...
                    className="px-3 py-2 sm:py-1.5 text-xs text-slate-400 active:text-white sm:hover:text-white transition-colors min-h-[40px] sm:min-h-0"
                  >
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 px-4 sm:px-3 py-2 sm:py-1.5 bg-teal-500 active:bg-teal-400 sm:hover:bg-teal-400 text-white rounded-lg text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed min-h-[40px] sm:min-h-0"
                  >
                    {isCommenting ? (
                      <Loader2 className="w-4 h-4 sm:w-3 sm:h-3 animate-spin" />
                    ) : (
      ...
      {c.replies && c.replies.length > 0 && (
        <div className={visualDepth > 0 ? "ml-4 sm:ml-8" : "ml-6 sm:ml-14"}>
          <button
            onClick={() => {
...
export default function Watch() {
  ...
  // Redirect clips to the clips feed
  useEffect(() => {
    if (video && video.isClip && video.publicId) {
      navigate(`/clips?v=${video.publicId}`, { replace: true });
    }
  }, [video, navigate]);
  ...
  // YouTube-style view tracking: count view after 5 seconds or 30% watched
  ...
  useEffect(() => {
    const autoPayForVideo = async () => {
      ...
      // MEMBERS-ONLY VIDEO - must wait for membership check, don't unlock based on price
      // Members-only videos are gated by membership, not by pay-per-view
      if (video.isMembersOnly) {
        // Wait for membership check to complete (hasMemberAccess will be set by separate useEffect)
        // Don't process payment - membership access is handled separately
        paymentProcessedForVideoRef.current = videoId;
        
        // If user is the owner, unlock immediately
        ...
        // Otherwise keep locked - the membership check useEffect will handle access
        // and the members-only overlay will show
        setPaymentState("locked");
        return;
      }
      
      // FREE VIDEO (non-members-only) - unlock immediately and record view on load
      ...
      // View will be recorded via handleTimeUpdate after 5 sec / 30% watch time (YouTube-style)
      if (channel && video.channel?.id === channel.id) {
  ...
  useEffect(() => {
    const checkMembershipAccess = async () => {
      ...
      // Need to be logged in to check membership
      const isLoggedIn = isConnected || !!externalWallet;
      if (!isLoggedIn) {
      ...
      try {
        // Build headers with auth for external wallet users
        const headers: Record<string, string> = {};
        if (externalWallet?.authToken) {
          headers["Authorization"] = `Bearer ${externalWallet.authToken}`;
        }
        
        const response = await fetch(`/api/kasshi/videos/${videoId}/access`, {
          credentials: 'include',
          headers,
        });
        const data = await response.json();
  ...
  }, [video, videoId, channel, isConnected, externalWallet]);
  
  // CRITICAL: Unlock members-only video when membership access is confirmed
  // This was missing - the membership check sets hasMemberAccess but nothing was unlocking the video
  useEffect(() => {
    if (video?.isMembersOnly && hasMemberAccess && !membershipLoading) {
      setPaymentState("unlocked");
      setRemainingTime(null);
      
      // Record view for members-only videos when member accesses
      if (!viewRecordedRef.current && videoId) {
        viewRecordedRef.current = true;
        fetch(`/api/kasshi/videos/${videoId}/view`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            viewerChannelId: channel?.id || null,
            transactionId: "membership-access",
            amount: 0,
            userId: externalWallet?.userId || user?.id,
          }),
        }).then(() => {
  ...
  }, [video?.isMembersOnly, hasMemberAccess, membershipLoading, videoId, channel?.id, externalWallet?.userId, user?.id]);
  ...
  // YouTube-style view recording: count after 5 seconds or 30% watched
    ...
    return (
      <div className={`min-h-screen w-full bg-slate-950 flex flex-col ${titleBarPadding}`}>
        ...
        <div className="flex flex-col items-center justify-center pt-40">
          ...
          <LocalizedLink to="/video" className="px-6 py-2 bg-teal-500 hover:bg-teal-400 text-white rounded-full font-medium transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="relative w-full rounded-xl overflow-hidden bg-black">
              ...
              {/* Use HLS when Bunny is finished, or fallback to direct R2 video for immediate playback */}
              {((video.bunnyStatus === 'finished' && video.videoUrl) || video.directVideoUrl) && paymentState === "unlocked" ? (
                <VideoPlayer
                  ref={playerRef}
                  src={video.bunnyStatus === 'finished' && video.videoUrl ? video.videoUrl : video.directVideoUrl!}
                  poster={thumbnail}
              ...
              {/* Encoding in progress overlay - only show if neither HLS nor direct video available */}
              {!video.videoUrl && !video.directVideoUrl && video.bunnyStatus && video.bunnyStatus !== 'finished' && video.bunnyStatus !== 'error' && paymentState === "unlocked" && (
              ...
              {video.isMembersOnly && hasMemberAccess === false && paymentState === "locked" && (
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/80 to-black/60 flex flex-col items-center justify-center p-6">
                  ...
                  <LocalizedLink
                    to={`/video/channel/${video.channel.handle}?tab=membership`}
            ...
            <div className="mt-3 sm:mt-4">
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-white leading-tight">{video.title}</h1>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-3 sm:gap-4 mt-3 sm:mt-4">
              {/* Channel info */}
              <div className="flex items-center gap-3 sm:gap-4">
                <LocalizedLink to={`/video/channel/${video.channel.handle}`}>
                  <img 
                    src={channelAvatar} 
                ...
                <div className="flex-1 min-w-0">
                  <LocalizedLink 
                    to={`/video/channel/${video.channel.handle}`}
                    className="font-semibold text-white hover:text-teal-400 transition-colors flex items-center gap-1 text-sm sm:text-base"
                  >
                ...
                {channel?.id === video.channel.id && (
                  <LocalizedLink
                    to={`/video/edit/${video.publicId || video.id}`}
                    className="px-3 sm:px-4 py-2 rounded-full font-medium text-xs sm:text-sm text-white bg-slate-700 active:bg-slate-600 sm:hover:bg-slate-600 flex items-center gap-1.5 sm:gap-2 transition-colors flex-shrink-0 min-h-[40px]"
                  >
                    <Edit3 className="w-4 h-4" /> <span className="hidden xs:inline">{t.video?.edit || 'Edit'}</span>
                  </LocalizedLink>
                )}
                {channel?.id !== video.channel.id && (
                  <button 
                    ...
                    className={`px-3 sm:px-4 py-2 rounded-full font-medium text-xs sm:text-sm transition-colors flex-shrink-0 min-h-[40px] ${
                      isSubscribed 
                        ? "bg-slate-700 text-white active:bg-slate-600 sm:hover:bg-slate-600" 
                        : "bg-white text-slate-900 active:bg-slate-200 sm:hover:bg-slate-200"
              ...
              <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 sm:overflow-visible scrollbar-hide">
                <div className="flex items-center bg-slate-800 rounded-full overflow-hidden flex-shrink-0">
                  <button 
                    onClick={handleLike}
                    disabled={isLiking}
                    className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 border-r border-slate-700 transition-colors active:bg-slate-700 sm:hover:bg-slate-700 min-h-[40px] ${
                      isLiked ? "text-teal-400" : "text-white"
                  ...
                  >
                    ...
                    <span className="text-xs sm:text-sm font-medium">{formatViews(localLikeCount)}</span>
                  </button>
                  <button 
                    onClick={handleDislike}
                    disabled={isDisliking}
                    className={`flex items-center gap-1 px-3 sm:px-4 py-2 transition-colors active:bg-slate-700 sm:hover:bg-slate-700 min-h-[40px] ${
                      isDisliked ? "text-rose-400" : "text-white"
                  ...
                  >
                    ...
                    <span className="text-xs sm:text-sm font-medium">{formatViews(localDislikeCount)}</span>
                  </button>
                </div>
                ...
                <div className="relative flex-shrink-0" data-dropdown>
                  <button 
                    ...
                    className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 bg-slate-800 active:bg-slate-700 sm:hover:bg-slate-700 rounded-full text-white transition-colors min-h-[40px]"
                  >
                    <Share2 className="w-5 h-5 text-teal-400" />
                    <span className="text-xs sm:text-sm hidden xs:inline font-medium">{t.video.share}</span>
                  </button>
                  {showShareMenu && (
                ...
                <button 
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 bg-gradient-to-r from-teal-500 to-emerald-500 active:from-teal-400 active:to-emerald-400 sm:hover:from-teal-400 sm:hover:to-emerald-400 rounded-full text-white font-medium transition-all min-h-[40px] flex-shrink-0"
                >
                  <Gift className="w-5 h-5" />
                  <span className="text-xs sm:text-sm">{t.video.tip}</span>
                </button>
                ...
                <div className="relative flex-shrink-0" data-dropdown>
                  <button 
                    ...
                    className="p-2 bg-slate-800 active:bg-slate-700 sm:hover:bg-slate-700 rounded-full text-white transition-colors min-w-[40px] min-h-[40px] flex items-center justify-center"
                  >
            ...
            <div className="mt-6 sm:mt-8">
              ...
              {/* Comment input */}
              <form onSubmit={handleComment} className="flex gap-2 sm:gap-4 mb-6 sm:mb-8">
                <img 
                ...
                <div className="flex-1">
                  <input
                    ...
                    className="w-full bg-transparent border-b border-slate-700 focus:border-teal-500 pb-2 text-white text-sm sm:text-base placeholder-slate-500 focus:outline-none transition-colors"
                    disabled={isCommenting}
                  />
                  {comment && (
                    <div className="flex justify-end gap-2 mt-2">
                      <button 
                        type="button"
                        onClick={() => setComment("")}
                        className="px-3 sm:px-4 py-2 text-xs sm:text-sm text-slate-400 active:text-white sm:hover:text-white transition-colors min-h-[40px] sm:min-h-0"
                      >
                        {t.common.cancel}
                      </button>
                      <button 
                        type="submit"
                        disabled={isCommenting}
                        className={`flex items-center gap-2 px-4 py-2 bg-teal-500 active:bg-teal-400 sm:hover:bg-teal-400 text-white rounded-full text-xs sm:text-sm font-medium transition-colors min-h-[40px] sm:min-h-0 ${isCommenting ? "opacity-50 cursor-wait" : ""}`}
                      >
                        {isCommenting ? (
              ...
              {commentsLoading ? (
                ...
                <div className="space-y-4 sm:space-y-6">
                  {comments.map((c) => {
                    ...
                    return (
                    <div key={c.id} className="space-y-2 sm:space-y-3">
                      {/* Main comment */}
                      <div className="flex gap-2 sm:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 sm:gap-2">
                            <span className="font-medium text-white text-sm truncate">{c.author.name}</span>
                            <span className="text-slate-500 text-xs flex-shrink-0">{formatTimeAgo(c.createdAt)}</span>
                          </div>
                          <p className="text-slate-300 text-sm mt-1 break-words">{c.content}</p>
                          <div className="flex items-center gap-1 sm:gap-4 mt-2 -ml-1.5 sm:ml-0">
                            <button 
                              onClick={() => handleCommentLike(String(c.id), c.author.walletAddress, c.author.name)}
                              disabled={likingCommentId === String(c.id)}
                              className={`flex items-center gap-1 transition-colors p-1.5 sm:p-0 min-h-[36px] sm:min-h-0 ${likingCommentId === String(c.id) ? "opacity-50 cursor-wait" : ""} ${topLevelIsLiked ? "text-teal-400" : "text-slate-400 active:text-teal-400 sm:hover:text-teal-400"}`}
                              title={topLevelIsLiked ? "Click to unlike" : "Like"}
                            >
                              {likingCommentId === String(c.id) ? (
                                <Loader2 className="w-5 h-5 sm:w-4 sm:h-4 animate-spin" />
                              ) : (
                                <ThumbsUp className={`w-5 h-5 sm:w-4 sm:h-4 ${topLevelIsLiked ? "fill-current" : ""}`} />
                              )}
                            ...
                            <button 
                              onClick={() => handleCommentDislike(String(c.id))}
                              disabled={dislikingCommentId === String(c.id)}
                              className={`flex items-center gap-1 transition-colors p-1.5 sm:p-0 min-h-[36px] sm:min-h-0 ${dislikingCommentId === String(c.id) ? "opacity-50 cursor-wait" : ""} ${topLevelIsDisliked ? "text-orange-400" : "text-slate-400 active:text-orange-400 sm:hover:text-orange-400"}`}
                              title={topLevelIsDisliked ? "Click to undislike" : "Dislike"}
                            >
                              {dislikingCommentId === String(c.id) ? (
                                <Loader2 className="w-5 h-5 sm:w-4 sm:h-4 animate-spin" />
                              ) : (
                                <ThumbsDown className={`w-5 h-5 sm:w-4 sm:h-4 ${topLevelIsDisliked ? "fill-current" : ""}`} />
                              )}
            ...
            {filteredRelated.length > 0 ? (
              <div className="space-y-4">
                {filteredRelated.map((v) => (
                  <LocalizedLink key={v.id} to={`/video/watch/${v.publicId || v.id}`} className="flex gap-3 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <PriceBadge priceKas={v.priceKas} />
                      </div>
                    </div>
                  </LocalizedLink>
                ))}
