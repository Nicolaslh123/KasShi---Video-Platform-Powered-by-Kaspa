...
export function BuySharesModal({ isOpen, onClose, track, onPurchaseComplete }: BuySharesModalProps) {
  ...
  const minSharesRequired = Math.ceil(MIN_INVESTMENT_KAS / pricePerShare);
  ...
  const handleSubmit = () => {
    ...
    if (totalCost < MIN_INVESTMENT_KAS) {
      setError(`Minimum investment is ${MIN_INVESTMENT_KAS} KAS (at least ${minSharesRequired} shares at this price)`);
      return;
    }
