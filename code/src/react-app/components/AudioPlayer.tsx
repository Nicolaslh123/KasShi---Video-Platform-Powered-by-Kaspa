...
import { 
  ...
  MoreHorizontal, Clock, Link, Radio, ListPlus, User, ListMusic, FolderPlus, X
} from 'lucide-react';
import QueuePanel from './QueuePanel';
import { useAudioVisualization } from '../contexts/AudioVisualizationContext';
...
export interface AudioTrack {
  ...
  reviewCount?: number;
  isReviewed?: boolean;
  beatGrid?: number[];  // pre-computed beat timestamps in seconds
...
interface AudioPlayerProps {
  ...
  // Close/exit the player completely
  onClose?: () => void;
}
...
export default function AudioPlayer({
  ...
  const { audioRef: contextAudioRef } = useAudioPlayer();
  
  // Sync local audioRef to context for visualizers to access
  useEffect(() => {
    if (contextAudioRef) {
  ...
  // Audio visualization context for reactive themes
  const { registerAnalyser, unregisterAnalyser, setIsPlaying: setVisualizationPlaying, setBeatGridData, setCurrentTime: setVisualizationCurrentTime } = useAudioVisualization();
  ...
  const initializeWebAudio = useCallback(() => {
    ...
    try {
      ...
      // Create analyser for visualization (reactive themes)
      const analyser = ctx.createAnalyser();
      analyser.fftSize = 2048; // High resolution for accurate bass frequency detection
      analyser.smoothingTimeConstant = 0.25; // Low smoothing to catch percussive transients instantly
      analyserRef.current = analyser;
      
      // Connect the chain: source -> bass -> treble -> compressor -> gain -> analyser -> destination
      ...
      // Register analyser with visualization context for reactive themes
  ...
  // Update beat grid data for reactive themes when track changes
  useEffect(() => {
    if (track?.beatGrid && track?.bpm) {
      setBeatGridData(track.beatGrid, track.bpm);
    } else {
      setBeatGridData(null, null);
    }
  }, [track?.id, track?.beatGrid, track?.bpm, setBeatGridData]);
  ...
  useEffect(() => {
    const audio = audioRef.current;
    // Initialize Web Audio if:
    // 1. We don't already have an AudioContext
    // 2. We have a track
    // 3. CRITICAL: The audio element already has a src loaded (to avoid createMediaElementSource issues)
    // Note: We initialize even with enhancement='off' because reactive themes need the analyser
    if (!audioContextRef.current && track && audio?.src) {
      initializeWebAudio();
    }
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      if (!isDragging) {
        ...
        setCurrentTime(time);
        setVisualizationCurrentTime(time);  // Update beat sync context
    ...
    const handleLoadedMetadata = () => {
      ...
      // Initialize Web Audio when metadata loads (audio is now ready)
      // This is needed for reactive themes even with audio enhancement off
  ...
  }, [isDragging, repeatMode, playlist.length, currentIndex, queue.length, onProgress, onPlayStateChange, onPlayFromQueue, handleNext, track, onPurchaseRequired, setVisualizationPlaying, initializeWebAudio]);
          ...
          <div className="relative flex items-center justify-between h-full px-4 max-w-7xl mx-auto z-10">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <button 
                ...
                className="text-white/70 active:text-white sm:hover:text-white transition-colors p-2 sm:p-1 min-h-[40px] sm:min-h-0 flex items-center justify-center"
              >
                <SkipBack className="w-5 h-5 fill-current" />
              </button>
              <button onClick={togglePlay} className="text-white active:scale-95 sm:hover:scale-110 transition-transform p-2 sm:p-1 min-h-[40px] sm:min-h-0 flex items-center justify-center">
                {isPlaying ? <Pause className="w-7 h-7 sm:w-6 sm:h-6" /> : <Play className="w-7 h-7 sm:w-6 sm:h-6 fill-current" />}
              </button>
              <button 
                onClick={handleNext}
                className="text-white/70 active:text-white sm:hover:text-white transition-colors p-2 sm:p-1 min-h-[40px] sm:min-h-0 flex items-center justify-center"
              ...
              <div className="relative sm:hidden">
                <button 
                  ...
                  className="text-white/70 active:text-white p-2 min-h-[40px] flex items-center justify-center transition-colors"
                >
                ...
                {showMoreMenu && (
                  <div 
                    ref={moreMenuRef}
                    className="absolute bottom-full right-0 mb-2 w-48 bg-black/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden z-[70]"
                  >
                    <button
                      onClick={() => { track && onLike?.(track.id); setShowMoreMenu(false); }}
                      className="w-full px-4 py-3 text-left text-sm text-white/80 active:bg-white/10 flex items-center gap-3"
                    >
                      <ThumbsUp className={`w-4 h-4 ${track?.isLiked ? 'fill-current text-orange-500' : ''}`} />
                      {track?.isLiked ? 'Unlike' : 'Like'}
                    </button>
                    <button
                      onClick={() => { track && onAddToQueue?.(track); setShowMoreMenu(false); }}
                      className="w-full px-4 py-3 text-left text-sm text-white/80 active:bg-white/10 flex items-center gap-3"
                    >
                    ...
                    <button
                      onClick={() => { setShowQueue(true); setShowMoreMenu(false); }}
                      className="w-full px-4 py-3 text-left text-sm text-white/80 active:bg-white/10 flex items-center gap-3"
                    >
              ...
              <button onClick={() => setIsExpanded(true)} className="text-white p-2 sm:p-1 min-h-[40px] sm:min-h-0 flex items-center justify-center">
              ...
              {onClose && (
                <button 
                  onClick={onClose}
                  className="text-white/70 hover:text-white active:text-white p-2 sm:p-1 min-h-[40px] sm:min-h-0 flex items-center justify-center transition-colors"
                  title="Close player"
                >
                  <X className="w-5 h-5" />
                </button>
