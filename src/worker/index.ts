...
app.post("/api/kasshi/videos", async (c) => {
  const body = await c.req.json();
  const { channelId, title, description, videoUrl, thumbnailUrl, durationSeconds, isMembersOnly, isPrivate, priceKas, bunnyVideoId, bunnyStatus: requestedBunnyStatus } = body;
  ...
  const price = parseFloat(priceKas || '0');
  const finalPrice = price > 0 && price < 0.11 ? '0' : (priceKas || '0');
  ...
  await c.env.DB.prepare(`
    INSERT INTO videos (channel_id, title, description, video_url, thumbnail_url, duration_seconds, status, is_members_only, is_private, public_id, price_kas, bunny_video_id, bunny_status)
    VALUES (?, ?, ?, ?, ?, ?, 'published', ?, ?, ?, ?, ?, ?)
  `).bind(channelId, title, description || null, videoUrl || null, thumbnailUrl || null, durationSeconds || 0, isMembersOnly ? 1 : 0, isPrivate ? 1 : 0, publicId, finalPrice, bunnyVideoId || null, bunnyStatus).run();
...
app.post("/api/kasshi/videos/:id/view", async (c) => {
  ...
  if (viewerChannelId) {
    ...
    // Use try/catch because video_views has UNIQUE(channel_id, video_id)
    // and all channel-less users share channel_id=0, causing conflicts
    try {
      const existingView = await c.env.DB.prepare(`
        SELECT id FROM video_views WHERE user_id = ? AND video_id = ?
      ...
      if (existingView) {
        ...
        // Try to insert, but if another channel-less user already watched this video,
        // the unique constraint on (channel_id=0, video_id) will fail - that's ok
        await c.env.DB.prepare(`
      ...
      // Unique constraint violation - view tracking failed but that's non-critical
...
app.get("/api/kasshi/dashboard/videos", async (c) => {
  ...
  // Get all videos for this channel, ordered by most recent
...
app.get("/api/music/featured", async (c) => {
  try {
    ...
    const tracksResult = await c.env.DB.prepare(`
      SELECT t.*, mp.name as artist_name, mp.handle as artist_handle, mp.wallet_address as creator_wallet,
...
app.get("/api/music/search", async (c) => {
  ...
  try {
    ...
    const tracksResult = await c.env.DB.prepare(`
      ...
      LEFT JOIN music_profiles mp ON t.music_profile_id = mp.id
      LEFT JOIN albums a ON t.album_id = a.id
      WHERE t.is_published = 1 AND (t.title LIKE ? OR mp.name LIKE ?)
...
app.post("/api/music/tracks", async (c) => {
  try {
    ...
    const { title, description, audioUrl, coverArtUrl, durationSeconds, albumId, trackNumber, genre, lyrics, priceKas, isExplicit, chapters } = body;
    ...
    const result = await c.env.DB.prepare(`
      INSERT INTO tracks (music_profile_id, channel_id, album_id, title, description, audio_url, cover_art_url, duration_seconds, track_number, genre, lyrics, price_kas, is_explicit, is_published)
      VALUES (?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
    `).bind(
      musicProfileId, 
      albumId || null, 
...
app.patch("/api/music/tracks/:id", async (c) => {
  try {
    const trackId = parseInt(c.req.param("id"));
    const body = await c.req.json<{ priceKas?: string; title?: string }>();
...
app.get("/api/music/user/liked", async (c) => {
  ...
  let query = `
    SELECT t.*, a.title as album_title, mp.name as artist_name, mp.handle as artist_handle, mp.wallet_address as creator_wallet, mp.id as artist_id
    FROM track_likes tl
...
app.get("/api/music/user/history", async (c) => {
  ...
  const result = await c.env.DB.prepare(`
    SELECT t.*, a.title as album_title, mp.name as artist_name, mp.handle as artist_handle, mp.wallet_address as creator_wallet, mp.id as artist_id, tp.created_at as played_at
    FROM track_plays tp
...
app.get("/api/music/radio", async (c) => {
  ...
  const result = await c.env.DB.prepare(`
    SELECT t.*, a.title as album_title, mp.name as artist_name
    FROM tracks t
    LEFT JOIN albums a ON t.album_id = a.id
    LEFT JOIN music_profiles mp ON t.music_profile_id = mp.id
    ORDER BY RANDOM()
...
app.get("/api/music/discover/tracks", async (c) => {
  ...
  let query = `
    SELECT t.*, a.title as album_title, mp.name as artist_name, mp.genre as artist_genre,
...
app.get("/api/music/playlists/:id", async (c) => {
  ...
  const tracksResult = await c.env.DB.prepare(`
    SELECT t.*, pt.track_order, a.title as album_title, 
           mp.name as artist_name, mp.handle as artist_handle, mp.id as artist_id, mp.wallet_address as creator_wallet
...
app.post("/api/marketplace/themes", async (c) => {
  try {
    ...
    const body = await c.req.json();
    const { title, description, previewImageUrl, themeData, priceKas, quantityTotal, hasParticles, particleColor } = body;
    ...
    if (!description || description.length < 20 || description.length > 500) {
      return c.json({ error: "Description must be 20-500 characters" }, 400);
    }
    if (!previewImageUrl) {
    ...
    const result = await c.env.DB.prepare(`
      INSERT INTO marketplace_themes (
        ...
        has_particles, particle_color, approval_status, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'pending', ?, ?)
    `).bind(
      walletAddress,
      musicProfileId,
...
app.get("/api/marketplace/themes", async (c) => {
  try {
    const themes = await c.env.DB.prepare(`
      SELECT 
        t.id, t.title, t.description, t.preview_image_url, t.theme_data,
        t.price_kas, t.quantity_total, t.quantity_sold, t.has_particles,
        t.particle_color, t.creator_wallet_address, t.created_at,
        mp.display_name as creator_name, mp.id as creator_profile_id
  ...
  try {
    const themes = await c.env.DB.prepare(`
      SELECT 
        ...
        t.creator_wallet_address, t.created_at, t.approval_status,
...
app.post("/api/marketplace/admin/themes/:id/approve", adminMiddleware, async (c) => {
  try {
    ...
    const theme = await c.env.DB.prepare(
      "SELECT id, approval_status FROM marketplace_themes WHERE id = ?"
    ).bind(themeId).first();
    ...
    if (theme.approval_status !== 'pending') {
      return c.json({ error: "Theme is not pending approval" }, 400);
    }
...
app.post("/api/marketplace/admin/themes/:id/reject", adminMiddleware, async (c) => {
  try {
    ...
    await c.env.DB.prepare(`
      UPDATE marketplace_themes 
      SET approval_status = 'rejected', rejection_reason = ?, updated_at = ?
      WHERE id = ?
...
// POST /api/marketplace/listings - Create a resale listing
app.post("/api/marketplace/listings", async (c) => {
  try {
    ...
    const { purchaseId, priceKas } = await c.req.json();
    ...
    // Create listing
    const result = await c.env.DB.prepare(`
      INSERT INTO theme_listings (theme_id, purchase_id, seller_wallet_address, seller_user_id, price_kas, is_auction, is_active)
      VALUES (?, ?, ?, ?, ?, 0, 1)
    `).bind(purchase.theme_id, purchaseId, auth.walletAddress || null, auth.userId || null, priceKas || '0').run();
    
    return c.json({ 
      success: true, 
      listingId: result.meta.last_row_id,
      message: "Theme listed for sale"
    });
  } catch (error) {
  ...
  try {
    const listings = await c.env.DB.prepare(`
      SELECT 
        ...
        t.id as theme_id, t.title, t.description, t.preview_image_url, 
  ...
  try {
    ...
    const listings = await c.env.DB.prepare(`
      SELECT 
        ...
        t.id as theme_id, t.title, t.preview_image_url, t.has_particles
      FROM theme_listings tl
      JOIN marketplace_themes t ON tl.theme_id = t.id
