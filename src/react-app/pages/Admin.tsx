...
import { ArrowLeft, Flag, Trash2, X, AlertTriangle, Video, User, Clock, Loader2, CheckCircle, Shield, Wallet, Copy, Check, DollarSign, Users, Radio, Film, AlertCircle, ExternalLink, HardDrive, RefreshCw } from "lucide-react";
...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<"reports" | "payouts" | "channels" | "videos">("reports");
  ...
  if (loading) {
    return (
      <div className="min-h-screen w-full bg-[#0a0f14] flex items-center justify-center">
  ...
  if (!adminStatus?.isAdmin) {
    return (
      <div className="min-h-screen w-full bg-[#0a0f14] flex flex-col items-center justify-center gap-4">
        ...
        <Link to="/" className="text-teal-400 hover:underline">Return to Home</Link>
      </div>
    );
  }

  return (
    <div className="min-h-screen w-full bg-[#0a0f14] flex flex-col">
      {/* Header */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#0a0f14] border border-white/10 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-4">
              {/* Video Preview */}
              <Link
                to={`/watch/${selectedReport.videoId}`}
                target="_blank"
